...
CONTROL_HORIZON = 6  # seconds. if this is set too high and INTEGRATION_DT is too low, code will take a long time to run!
...
def load_map(filename):
    ...
    print(os.path.exists("../maps/" + filename))
...
class PathFollower():
    ...
    def collision_detected(self, robot_traj):
        ...
        valid_indices = (col_indices >= 0) & (col_indices < self.map_np.shape[1]) & (row_indices >= 0) & (row_indices < self.map_np.shape[0])
        col_indices, row_indices = col_indices[valid_indices], row_indices[valid_indices]
